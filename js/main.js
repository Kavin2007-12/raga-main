...
function setupGlobalEventListeners() {
  document.addEventListener("click", (event) => {
    ...
    // 3. Quick View toggle delegation (Redirects directly to Buy Now flow)
...
function openQuickView(productId) {
  ...
  content.innerHTML = `
    ...
    <div class="p-8 flex flex-col justify-between h-[450px] overflow-y-auto">
      ...
      <div class="pt-2">
        <button onclick="buyNow('${product.id}')" class="w-full flex items-center justify-center bg-brand-burgundy hover:bg-brand-burgundyLight text-white text-xs uppercase tracking-widest font-bold py-3.5 transition duration-300 border-b-2 border-brand-gold shadow-md">
          BUY NOW
        </button>
      </div>
    </div>
