      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
          ...
          <div class="hidden md:flex flex-1 items-center gap-3 mx-6">
            ...
            <a href="javascript:void(0)" onclick="openCustomerLoginModal()" id="header-login-btn" class="whitespace-nowrap text-[10px] uppercase tracking-[0.12em] font-bold px-3.5 py-2.5 border border-brand-burgundy text-brand-burgundy hover:bg-brand-burgundy hover:text-white transition-all duration-200 rounded-sm">Login</a>
            <a href="javascript:void(0)" onclick="openCustomerSignupModal()" id="header-signup-btn" class="whitespace-nowrap text-[10px] uppercase tracking-[0.12em] font-bold px-3.5 py-2.5 bg-brand-burgundy text-white hover:bg-brand-burgundyLight transition-all duration-200 rounded-sm border-b-2 border-brand-gold">Sign Up</a>
            <a href="admin.html" id="header-admin-btn" class="whitespace-nowrap text-[10px] uppercase tracking-[0.12em] font-bold px-3.5 py-2.5 bg-brand-charcoal text-brand-gold hover:bg-black transition-all duration-200 rounded-sm border-b-2 border-brand-gold flex items-center gap-1">
            ...
            <div class="relative group">
              ...
              <div class="absolute right-0 mt-2 w-72 bg-white border border-gray-100 shadow-2xl p-6 hidden group-hover:block z-50 text-left rounded-lg transition-all duration-300 animate-toast">
                ...
                <div class="space-y-4 text-xs font-semibold text-brand-charcoal/90 mb-6">
                  ...
                  <a href="admin.html" class="block text-brand-burgundy font-bold hover:underline transition pt-2 border-t border-gray-100 flex items-center gap-1.5">
                    <span>👑</span> <span>Go to Admin Panel</span>
                  </a>
                </div>

                <!-- User Customer Login Button -->
                <a href="javascript:void(0)" onclick="openCustomerLoginModal()" class="w-full flex items-center justify-center space-x-2 bg-brand-burgundy hover:bg-brand-burgundyLight text-white text-xs uppercase tracking-widest font-bold py-3 px-4 rounded-full transition duration-300">
    ...
    <footer class="bg-brand-cream border-t border-brand-gold/15 pt-16 pb-8 text-brand-charcoal">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 pb-12 border-b border-brand-gold/15">
          ...
          <div>
            ...
            <div class="flex space-x-3 pt-4">
              ...
              <a href="https://www.instagram.com/ragaboutique_?igsh=MXdqdmczNXdkMHIyeQ%3D%3D&utm_source=qr" target="_blank" rel="noopener noreferrer" class="p-2 bg-brand-burgundy text-white hover:bg-brand-burgundyLight transition duration-300 rounded-full w-8 h-8 flex items-center justify-center" aria-label="Instagram">
        ...
        <div class="pt-8 flex flex-col md:flex-row justify-between items-center text-xs font-semibold text-brand-charcoal/60 mt-6 border-t border-brand-gold/15">
          <p>© 2026 Raga Boutique E-commerce. All Rights Reserved. • <a href="admin.html" class="text-brand-burgundy font-bold hover:underline">👑 Admin Panel</a></p>
...
window.openCustomerSignupModal = function() {
  const modal = document.getElementById('admin-login-modal');
  if (modal) {
    const title = modal.querySelector('h2');
...
window.handleAdminLoginSubmit = async function(e) {
  ...
  const userInput = document.getElementById('admin-user-modal');
  const passInput = document.getElementById('admin-pass-modal');
  ...
  let storedAuth = { username: 'admin', passwordHash: '8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918' };
  try {
    const s = localStorage.getItem('raga_admin_auth_v2');
    if (s) storedAuth = JSON.parse(s);
  } catch(e){}
  ...
  try {
    ...
    enteredHash = hashArray.map(b => b.toString(16).padStart(2, '0')).join('');
  } catch(e) {
    let hash = 0;
    for (let i = 0; i < p.length; i++) { hash = (hash << 5) - hash + p.charCodeAt(i); hash |= 0; }
    enteredHash = 's256_' + Math.abs(hash).toString(16);
  }

  if (u === storedAuth.username && enteredHash === storedAuth.passwordHash) {
    if (err) err.classList.add('hidden');
    localStorage.removeItem('raga_admin_explicit_logout');
    sessionStorage.setItem('raga_admin_session', 'authenticated');
    window.location.href = 'admin.html';
  } else {
    if (err) {
      err.textContent = 'Invalid username or password.';
